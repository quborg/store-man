...
const REG = {
  ...
  phone     : p   => /^([0-9][\s,\-./*]?){10}$/.test(p.trim()),
  email     : e   => validator.isEmail(e.trim()),
  adress    : a   => /^[a-zA-Z0-9\s,'-]+$/.test(a.trim()),
  client_id : id  => validator.isMongoId(id.trim()),
  array     : arr => arr.length,
}

/**
 * validateFields. form validator and error flag firer
 * @param Object fields
 * @param Object errorsFlag
  ...
  if (keys.length) {
    keys.map( key => {
      if (fields.hasOwnProperty(key) && key in errorsFlag) {
        const validate = (EX, data=fields[key]) => REG[EX](data)
        let keyIsValid = true
        switch (key) {
          case 'email'    : keyIsValid = validate('email') || !fields[key];       break
          case 'phone'    :
          case 'adress'   :
          case 'client_id': keyIsValid = validate(key);     break
          case 'basket'   : keyIsValid = validate('array', fields[key].products); break
          case 'products' : keyIsValid = validate('array'); break
          default         : keyIsValid = validate('name');
        }
        errorsFlag[key] = keyIsValid ? '' : 'error'
        !keyIsValid && ( errorRuntime = true )
      }
    })
  }
