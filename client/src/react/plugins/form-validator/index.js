...
const REG = {
  ...
  phone     : p   => /^([0-9][\s,\-./*]?){10}$/.test(p),
  email     : e   => validator.isEmail(e),
  adress    : a   => /^[a-zA-Z0-9\s,'-]+$/.test(a),
  client_id : id  => (typeof id == 'string') && validator.isMongoId(id),
  array     : arr => arr.length,
  image     : i   => i !== 'error-file-type'
}
  ...
  if (keys.length) {
    keys.map( key => {
      if (fields.hasOwnProperty(key) && key in errorsFlag) {
        ...
        switch (key) {
          case 'image'    :
          case 'email'    : keyIsValid = validate(key) || !fields[key];       break
          case 'phone'    :
          case 'adress'   :
